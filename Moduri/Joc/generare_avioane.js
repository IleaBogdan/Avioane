...
function initializare_SA()
{
    ...
    for (var i = 0; i < 4; i++) {
        SA[i] = {};
        SA[i].mask = new Array(5);
        for (var j = 0; j < 5; j++) {
            SA[i].mask[j] = new Array(5);
            for (var k = 0; k < 5; k++) {
                SA[i].mask[j][k] = ' ';
            }
        }
        SA[i].direction = orientari[i];
        console.log(SA[i].direction);
    }
    ...
    for (var k = 1; k < 4; k++) {
        for (var i = 0; i < 5; i++)
        for (var j = 0; j < 5; j++) {
            SA[k].mask[j][5 - i - 1] = SA[k - 1].mask[i][j];
        }
    }
    for (var k = 0; k < 4; k++) {
        for (var i = 0; i < 5; i++)
            for (var j = 0; j < 5; j++) {
                if (SA[k].mask[i][j] === 'X') SA[k].mask[i][j] = SA[k].direction; 
            }
    }
    for (var k = 0; k < 4; k++) {
        console.log(JSON.stringify(SA[k].mask));
    }
